...
use crate::{delay_us, embedded_mmc::{card::CardType, commands::{DataBuffer, MmcCommand, MmcResponse}, host::constants::*}};

use super::{err::SdError, EMmcHost};
...
impl EMmcHost {
    ...
    pub fn mmc_send_op_cond(&mut self, ocr: u32, mut retry: u32) -> Result<u32, SdError> {
        ...
        while retry > 0 && !ready {
            ...
            info!("CMD1 response raw: {:#x}", self.read_reg32(EMMC_RESPONSE));
